...
func (kub *Kubectl) DeleteETCDOperator() error {
	...
	kub.Exec("kubectl delete deployment,pods,etcdclusters.etcd.database.coreos.com -n kube-system -l io.cilium/app=etcd-operator")
	if !cmdRes.WasSuccessful() {
		return fmt.Errorf("Unable to delete deployment and pods with label io.cilium/app=etcd-operator: %s", cmdRes.OutputPrettyPrint())
	}
	return nil
}
